...
THREEx.DayNight.SunLight	= function(){
	...
	// light.shadowMapCullFrontFaces = true;
    // light.shadow.mapSize.width = 12096;
    // light.shadow.mapSize.height = 12096;
...
targetObject.translateX(110)
targetObject.translateZ(110)
...
// scene.add( helper );
	...
	this.update	= function(sunAngle){
		...
		// if(sunAngle > 3.14 && sunAngle < 6.28){
		...
		light.position.y = Math.sin(sunAngle) * 350;
		light.position.z = 110 + (Math.cos(sunAngle) * 350);
// console.log('Phase ', THREEx.DayNight.currentPhase(sunAngle))
		...
		if( phase === 'day' ){
			...
			light.intensity	= 0.3;
      light.color.set("rgb(" + 255 + "," + 255 + ",255)");
		}
...
THREEx.DayNight.MoonLight	= function(){
	var light	= new THREE.DirectionalLight( 0xffffff, 1 );
  	light.castShadow = true;
	light.shadowMapCullFrontFaces = true;
    light.shadow.mapSize.width = 12096;
...
scene.add( helper );
	...
	this.update	= function(sunAngle){
		light.position.x = 0;
		light.position.y = Math.sin(sunAngle) * -350;
		light.position.z = Math.cos(sunAngle) * -350;
// console.log('Phase ', THREEx.DayNight.currentPhase(sunAngle))

		var phase	= THREEx.DayNight.currentPhase(sunAngle)
		if( phase === 'day' ){
			light.color.set("rgb(0,"+ (Math.floor(Math.sin(sunAngle)*200)+55) + "," + (Math.floor(Math.sin(sunAngle)*200)) +")");
		}else if( phase === 'twilight' ){
		        light.intensity = 1;
	        	light.color.set("rgb(0," + (55-Math.floor(Math.sin(sunAngle)*110*-1)) + ","+(255-Math.floor(Math.sin(sunAngle)*510*-1))+")");
		} else {
			light.intensity	= 0.3;
      light.color.set("rgb(" + 255 + "," + 255 + ",255)");
		}
