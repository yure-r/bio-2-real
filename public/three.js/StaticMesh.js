...
(function() {
	...
	THREE.Frustum.prototype.intersectsObject = function () {
		...
		return function intersectsObject( object ) {
			...
			if ( object.isStatic ) {
				...
				if ( object.isChild && object.parent.compound )
				...
				if ( !object.visible && object.compound ) {
	...
	THREE.StaticMesh = function( source, parent ) {
		...
		this.id = id -- ; // THREE hides the Object3D id counter so we use a negative
		this.index = 0;
		this.parentIndex = 0;
	...
	THREE.StaticMesh.prototype = {
		...
		// For compatibility with 126+ releases

		updateMorphTargets: THREE.Mesh.prototype.updateMorphTargets,
		onBeforeRender: THREE.Mesh.prototype.onBeforeRender,
		onAfterRender: THREE.Mesh.prototype.onAfterRender,
			...
			if ( this.isChild ) {
				...
				target.traverse( object => {
					...
					if ( index > -1 ) {
						...
						matrix.copy( parent.matrixWorld ).invert();
		...
		save: function() {
			...
			source.traverse( object => {
				...
				if ( index > -1 ) {
					...
					if ( !source.compound && child.geometry ) {
			...
			if ( this.compound && this.children.length )
